...
description: |-
  ...
  ```hcl
  resource "guancemute" "alertpolicy" {
    name        = "terraform-alert-policy-mute"
    description = "Managed by Terraform"
    type        = "alertPolicy"
    timezone    = "Asia/Shanghai"
  muteranges = [{
      name              = guancealertpolicy.example.name
      alertpolicyuuid = guancealert_policy.example.uuid
    }]
  repeattimeset = 0
  ...
  ```hcl
  resource "guancemute" "withnotify" {
  ...
  notifytargets = [{
      type = "notifyObject"
      to   = [guancenotify_object.example.uuid]
    }]
  }
  ```
  Argument Reference
  ...
  output "mutedalertpolicyuuid" {
    value = data.guancemute.example.muteranges[0].alertpolicy_uuid
  }
  ```
  Lookup by UUID:
  hcl
